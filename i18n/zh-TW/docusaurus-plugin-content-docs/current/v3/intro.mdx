...
:::tip
- 由於曆史原因，存在兩種籤名方式：一種[籤名在請求頭](./intro#籤名方式一籤名在請求頭)中，另一種[籤名在請求體](./intro#籤名方式二籤名在請求體)中；使用哪種籤名方式請參考具體接口請求示列。
...
| 參數 | 類型 | 說明|
|:----- |:-----|----- |
|ret_code / retCode |number |成功/錯誤碼 |
|ret_msg / retMsg |string |成功/錯誤消息. 成功消息可以是`OK`,`success`,`SUCCESS`|
|result |Object |業務數據體|
...
:::tip
- 由於曆史原因，[查詢實時委托單接口](./order/open-order/#)的響應體中result對象中的list對象裡返回的屬性是大冩字母開頭，對接時請注意！
:::